...
- 多节点（集群）多副本模式-同步双写：多个master多个slave，主从之间**同步复制**
...
修改4台服务器目录/data/rocketmq/conf/2m-2s-sync下的四个文件(
broker-a.properties,broker-a-s.properties,broker-b.properties,broker-b-s.properties)，以broker-a.properties为基础，其余配置文件参数微调
...
# 查消费进度
sh mqadmin consumerProgress -n 127.0.0.1:9876 -g xx_topic_consumer_group -s true
