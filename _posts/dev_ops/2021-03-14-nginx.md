...
#### 方式2：logrotate配置切割Nginx日志
Nginx日志管理之日志归档配置: https://blog.csdn.net/qq_37278522/article/details/139853068
``` 
...
### 4-6 四层、七层与DNS负载均衡　
- nginx.4——正向代理和反向代理（七层代理和四层代理）：https://blog.csdn.net/qq_61843057/article/details/133868688

``` 
nginx配置4层代理和7层代理
Nginx可以作为4层代理（通常指的是TCP代理），也可以作为7层代理（通常指的是HTTP代理）。

# 4层代理配置（TCP）：
在Nginx中配置4层代理通常涉及到使用stream模块。以下是一个简单的配置示例，它将Nginx配置为一个TCP负载均衡器：
...
# 7层代理配置（HTTP）：
在Nginx中配置7层代理通常涉及到使用http模块。以下是一个简单的配置示例，它将Nginx配置为一个HTTP负载均衡器：
...
在这个配置中，Nginx监听80端口，并将接收到的HTTP请求代理到名为backend的上游组，上游组中包含了两个后端服务器。代理服务器会将客户端的请求按照轮询方式分发到后端服务器
```
