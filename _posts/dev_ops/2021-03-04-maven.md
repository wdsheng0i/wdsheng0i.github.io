...
- Mvn compile 编译
- Mvn clean 清空编译生成的文件 //动过配置文件或者pom文件后需要maven update->maven clean 动过的项目的pom->maven
  install重新发布一下
...
而一些比较细致一点的需要根据项目的不同来定义的就需要定义在项目的pom.xml中。具体而言，能够定义在settings.xml中的信息有<repositories>、<properties>
和<pluginRepositories>定义在<properties>里面的键值对可以在pom.xml中使用。
