...
- 文档地址：
- Jenkins文档 https://www.k8stech.net/jenkins-docs/
...
因为xjar 支持windows平台和linux平台,所以需要实现用xjar.go 来生成
xjar文件的启动器,启动器与xjar文件具有一一对应的关系,防止文件包被篡改,即,每次mvn打包生成的xjar.go和xjar包一一对应,即便密码相同也不能通用
