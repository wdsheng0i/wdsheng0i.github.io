...
- 仓库类型：
    - proxy：是远程仓库的代理。比如说在nexus中配置了一个central
    ...
    - Group：是仓库组，在maven里没有这个概念，是nexus特有的。目的是将上述多个仓库聚合，对用户暴露统一的地址，这样用户就不需要在pom中配置多个地址，只要统一配置group的地址就可以了右边那个
      Repository Path 可以点击进去，看到仓库中artifact列表, 不过要注意浏览器缓存。
...
7.Build path is incomplete. Cannot find class file for com.**.**Configuration    **.xml /**xt/src/spring line 5 Spring
AOP Problem
...
10.-Dmaven.multiModuleProjectDirectory system propery is not set. Check $M2_HOME environment variable and mvn script
match.
