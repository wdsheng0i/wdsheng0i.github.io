...
- 监控平台：Prometheus + Grafana 
  ...
  - 青牛踏雪的博客prometheus：https://www.prometheus.wang/
...
## 三、docker部署 Prometheus
``` 
docker run -p 9090:9090 -v /etc/prometheus/prometheus.yml:/etc/prometheus/prometheus.yml prom/prometheus
```

## 四、k8s部署 Prometheus + Grafana 
...
## 五、部署xx-Exporter采集主机、中间件、集群运行数据
### 5.1 离线tar包部署Node Exporter采集主机运行数据
``` 
