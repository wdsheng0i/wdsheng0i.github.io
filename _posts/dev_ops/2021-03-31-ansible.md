...
template 模块使用了 jinjia2 格式作文文件模板 , 可以进行文档内变量的替换 ，它的每次使用都会被 ansible 标记为 “changed”
状态 , 文件以 .j2 结尾 , 常用参数如下
...
处理程序handlers与任务task完全相同（它可以做task可以做的任何事），但只有当另一个任务调用（通过notify）它时才会运行。您可以将其视为事件系统的一部分;
处理程序将通过其侦听的事件调用进行操作。  
...
一个 Playbook 是由一个或多个 Play 构成，Play 中的每一个 key , 比如 key1 , key2 等 , 这些 key 在 PlayBook 中被定义为 Play
的属性
...
此外，真实的配置通常需要额外的数据，如变量，文件，动态模板等等。这些工具可以与Playbook一起使用，可以通过将相关任务和数据组织成一个角色（role，
相关的结构）
